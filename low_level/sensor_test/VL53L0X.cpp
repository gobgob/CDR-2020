...
uint16_t VL53L0X::readRangeContinuousMillimeters(uint8_t &status)
{
  //startTimeout();
  ...
  uint8_t intflag = readReg(RESULT_INTERRUPT_STATUS);

  //uint8_t buf[12];
  //readMulti(RESULT_RANGE_STATUS, buf, 12);
  uint16_t range = readReg16Bit(RESULT_RANGE_STATUS + 10);
  status = readReg(RESULT_RANGE_STATUS);
  //uint8_t unknown = readReg(RESULT_RANGE_STATUS + 1);
  //uint16_t EffectiveSpadRtnCount = readReg16Bit(RESULT_RANGE_STATUS + 2);
  //uint16_t unknown2 = readReg16Bit(RESULT_RANGE_STATUS + 4);
  uint16_t SignalRate = readReg16Bit(RESULT_RANGE_STATUS + 6);
  //uint16_t AmbientRate = readReg16Bit(RESULT_RANGE_STATUS + 8);

  if (range < 2000 && SignalRate > 250) {
      Serial.print(intflag & 0x07);
      Serial.print("\t");
      Serial.print(status);
      //Serial.print("\t");
      //Serial.print(unknown);
      //Serial.print("\t");
      //Serial.print(EffectiveSpadRtnCount);
      //Serial.print("\t");
...
uint16_t VL53L0X::readRangeSingleMillimeters(void)
{
  ...
  uint8_t foo;
  return readRangeContinuousMillimeters(foo);
}
