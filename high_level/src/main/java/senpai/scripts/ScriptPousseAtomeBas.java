...
public class ScriptPousseAtomeBas extends Script
{
	...
	public ScriptPousseAtomeBas(Log log, Robot robot, Table table, CapteursProcess cp, OutgoingOrderBuffer out, boolean symetrie)
	{
		super(log, robot, table, cp, out);
		
		if(symetrie)
		{
			at = AtomeParTerre.BAS_GAUCHE;
			positionEntree.setX(- positionEntree.getX());
			angleEntree = Math.PI - angleEntree;
		}
		else
			at = AtomeParTerre.BAS_DROITE;
